	...
	struct filelistparser {
		...
		static inline const auto unix = [] {
			...
			auto len = _sprintf_p(
				...
				// 1. ファイル種別、2. パーミッション、(リンク)、3. Owner、スキップ、4. サイズ、(5. 月、6. 日、(7. 年|8. 時、9. 分)|10. 年、11. 月、12. 日)、13. ファイル名
				R"(^ *([-+dfl])([^ ]{9})(?:[0-9]+| +[0-9]+|) +([^ ]+) +.*?([0-9]+) +(?:%2$s[^ 0-9]* *%3$s[^ 0-9]* +(?:%1$s[^ 0-9]*|%4$s[^ 0-9]+%5$s[^ 0-9]*)|%1$s[^ 0-9]* +%2$s[^ 0-9]* +%3$s[^ 0-9]*) +(.*)$)",
		...
		static inline const auto dos = [] {
			...
			auto len = _sprintf_p(
				pattern, std::size(pattern),
				// (1. 年、2. 区切り、3. 月、4. 日 | 5. 月、6. 区切り、7. 日、8. 年) 、9. 時、10. 分、(11. 秒)?、12. AMPM、13. サイズ、14. ファイル名
